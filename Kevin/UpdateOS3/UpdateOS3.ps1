...
.RELEASENOTES
Version 1.5:  Modified Logging Path - Dave Kawula MVP
...
#Dave Kawula - Update --> We want this to work with AutoPilot so what I've done is created different versions of the App and packaged accordingly.
#We will use the Intune Force Reboot Option and rerun again.

#So basically it is a updateOS.PS1 - Win32APP V1 - Logs to $env:ProgramData)\Microsoft\UpdateOS
#Then We will just modify Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS.ps1.tag" -Value "Installed" to
#Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS1.ps1.tag" -Value "Installed"
#Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS2.ps1.tag" -Value "Installed"
#Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS3.ps1.tag" -Value "Installed"

#Each of the above would be subsequence reboots.   
...
#This is the first run so use this line
#Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS1.ps1.tag" -Value "Installed"

#This is the second run so use this line
#Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS2.ps1.tag" -Value "Installed"

#This is the third run so use this line
Set-Content -Path "$($env:ProgramData)\Microsoft\UpdateOS\UpdateOS3.ps1.tag" -Value "Installed"
